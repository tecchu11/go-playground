...
// UserRole represents user role to perform to request.
type UserRole int
...
type (
	// AuthUser represents pre authenticated user.
	AuthUser struct {
		Name string
		Role UserRole
	}
	// PreAuthenticatedUsers represents pre authenticated user map.
...
func (u PreAuthenticatedUsers) Auth(failure renderer.JSON, permitRoles map[UserRole]struct{}) func(http.Handler) http.Handler {
...
// CurrentUser retrieve from AuthUser from context.
